...
public class WorkItemController {
    ...
    public ResponseEntity<Map<String, Object>> createEntity(@RequestBody WorkItemDTO workItemDTO, Authentication authentication) {
        ApplicationContext context = this.applicationContextHelper.generateAppContext(authentication);

        try {

            TransactionResponse transactionResponse = workItemService.createOrUpdateEntity(workItemDTO, context);
            return transactionResponse.getResponseStatus().compareTo(TransactionResponse.RESPONSE_STATUS.FAILURE) == 0 ? ResponseEntity.badRequest().body(transactionResponse.errorMap()) : ResponseEntity.ok(transactionResponse.convertToMap());
        } catch (Exception ex) {
            log.error("Error while creating obj", ex);
            String error = ex.getMessage();
            Map<String, Object> mp = new HashMap();
            List<MomentusError> errors = new ArrayList();
            MomentusError momentusError = new MomentusError("10000", this.generalMessages.getMessage("10000", context.getLocale()));
    ...
    @GetMapping({"/getById"})
    public ResponseEntity<WorkItemDTO> getEntityById( @RequestParam Long id, Authentication authentication) {
        try {
            ApplicationContext context = this.applicationContextHelper.generateAppContext(authentication);
