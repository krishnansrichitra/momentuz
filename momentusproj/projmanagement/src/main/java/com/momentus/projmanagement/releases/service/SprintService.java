...
public class SprintService extends GenericService {
    ...
    public TransactionResponse generateSprints(ApplicationContext context, SprintGenerationDTO generationDTO){
        List<Sprint> sprints = generateSprintObjects (generationDTO,context);
        Integer count = 0;
        for (Sprint sprint : sprints){
            if ( sprint.getTeam() != null)
            {
               Team team = (Team) genericService.findById(sprint.getTeam().getId(), Team.class,context,true );
               if ( team !=null) {
                   sprint.setTeam(team);
            ...
            if (sprint.getRelease() != null){
                Release release = (Release) genericService.findById(sprint.getRelease().getId(), Release.class,context,true );
                if ( release != null) {
                    sprint.setRelease(release);
                    sprint.setProject(release.getProject());
                }
            }
            TransactionResponse response = genericService.adaptValidateAndSaveEntity(sprint,context,false);
            if (!response.hasHardError() && response.getResponseStatus().equals(TransactionResponse.RESPONSE_STATUS.SUCCESS)) {
                count++;
            }

        }
        TransactionResponse response =  new TransactionResponse(TransactionResponse.RESPONSE_STATUS.SUCCESS);
        response.setResponseMesage(generalMessages.getMessage(WorkItemErrorCodes.SPRINT_SAVED,new Object[]{String.valueOf(count)},context.getLocale()));
        return response;
    }

    private List<Sprint> generateSprintObjects(SprintGenerationDTO generationDTO,ApplicationContext context){
