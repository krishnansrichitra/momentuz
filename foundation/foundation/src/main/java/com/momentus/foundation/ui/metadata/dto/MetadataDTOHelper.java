        ...
        if (metadata != null) {
            ...
            if(!CollectionUtils.isEmpty(metadata.getUpdateViewFields()))
            {
                for(UpdateViewField updateViewField : metadata.getUpdateViewFields()) {
                    UpdateViewFieldDTO updateViewFieldDTO = new UpdateViewFieldDTO(updateViewField.getId(), updateViewField.getFieldKey(),
                            generalMessages.getMessage(updateViewField.getFieldKey(), locale), updateViewField.getControl(), updateViewField.getParam(),
                            updateViewField.getAccessor(), updateViewField.getVisibility());
           ...
           if(!CollectionUtils.isEmpty(metadata.getUpdateViewButtons())){
               for (UpdateViewButton updateViewButton : metadata.getUpdateViewButtons()){
                   UpdateViewButtonDTO updateViewButtonDTO = new UpdateViewButtonDTO(updateViewButton.getId(),updateViewButton.getButtonClass(),
                           updateViewButton.getJsMethod(),generalMessages.getMessage(updateViewButton.getInnerText(),locale),
                           updateViewButton.getVisibility(),updateViewButton.getSeqNo());
