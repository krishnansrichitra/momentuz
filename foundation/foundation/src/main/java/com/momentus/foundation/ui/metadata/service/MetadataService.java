...
public class MetadataService {
    ...
    @Autowired
    UpdateViewMetadataRepository updateViewMetadataRepository;
    ...
    public UpdateViewMetadataDTO getUpdateViewMetadata(Long orgId, String entity, Locale locale) {
        ...
        OrgProfile orgProfile = orgProfileService.getProfileForGroup("GNL", orgId);
        if (orgProfile != null) {
            List<UpdateViewMetadata> updateViewMetadatas = updateViewMetadataRepository.findByProfile_ProfileCodeInAndEntity(
                    Arrays.asList(orgProfile.getProfile().getProfileCode()),entity);
            if(!CollectionUtils.isEmpty(updateViewMetadatas)){
                return metadataDTOHelper.makeUpdateViewDTO(updateViewMetadatas.get(updateViewMetadatas.size() -1),locale);
            }
        }
        return null;
