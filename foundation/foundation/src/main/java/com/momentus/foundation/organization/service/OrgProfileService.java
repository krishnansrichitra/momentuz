...
public class OrgProfileService {
    ...
    public TransactionResponse save(OrgProfile orgProfile, ApplicationContext context)
    {
        if (context.getOrganization().getId() != 1) {
            MomentusError momentusError = new MomentusError(GeneralMessages.UNAUTHORIZED_SYSTEM_OPERATION,
                    generalMessages.getMessage(GeneralMessages.UNAUTHORIZED_SYSTEM_OPERATION,context.getLocale()));
            return  new TransactionResponse(TransactionResponse.RESPONSE_STATUS.FAILURE, Arrays.asList(momentusError),orgProfile);
        }
        ...
        OrgProfile curprofile = orgProfileRepository.findByProfileGroup_ProfileGroupCodeAndOrgId_Id(profileGroupCode,orgProfile.getOrgId().getId()) ;
        if(curprofile != null){
